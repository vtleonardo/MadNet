...
contract CryptoLibraryWrapper {
    ...
    function validateSignatureASM2(bytes calldata groupSignature_, bytes calldata bClaims_)
        public
        view
        returns (bool, uint256)
    {
        ...
        uint256 gasBefore = gasleft();
        return (
            CryptoLibrary.verifySignatureASM2(
                abi.encodePacked(keccak256(bClaims_)),
                signature,
                masterPublicKey
            ),
    ...
    function validateSignatureASM3(bytes calldata groupSignature_, bytes calldata bClaims_)
        public
        view
        returns (bool, uint256)
    {
        ...
        uint256 gasBefore = gasleft();
        return (
            CryptoLibrary.verifySignatureASM3(
                abi.encodePacked(keccak256(bClaims_)),
                signature,
                masterPublicKey
            ),
