...
func TestRegisterTask_Group_1_Task(t *testing.T) {
	...
	eth := testutils.GetEthereumNetwork(t, false, 5, "", nil)
	defer eth.Close()
...
//	eth := testutils.GetEthereumNetwork(t, false, 6, "", nil, nil)
//	assert.NotNil(t, eth)
//	defer eth.Close()
//
...
//	testutils.AdvanceTo(eth, event.StartBlock.Uint64()+event.PhaseLength.Uint64())
//
//	// get validator addresses
//	ctx, cf := context.WithTimeout(context.Background(), 10*time.Second)
//	defer cf()
